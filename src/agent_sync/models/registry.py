...
class ExternalSkill(BaseModel):
    """A skills.sh skill to vendor into .agents/skills/[<category>/]<local_name>/."""
    ...
    def validate_category(cls, value: str | None) -> str | None:
        """Reject category paths whose segments are not safe slugs."""

        if value is not None and not all(
            SAFE_SLUG_PATTERN.fullmatch(segment) for segment in value.split("/")
        ):
            raise ValueError(
                f"Invalid category '{value}' (each '/'-separated segment must match {SAFE_SLUG_PATTERN.pattern})"
            )
    ...
    def relative_path(self) -> Path:
        ...
        if self.category is None:
            return Path(self.local_name)

        return Path(*self.category.split("/"), self.local_name)
